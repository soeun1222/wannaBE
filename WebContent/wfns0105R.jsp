...
<body>
    ...
    <div id="show">
    	<form name="show_frm" action="wfns0104R.jsp?nf_num=<%= nf_num %>&u_id=<%= u_id %>&pageNum=<%= pageNum %>" method="post">
            <p class="title"><span class="font-color_main">비대면 이야기</span> 게시글</p>
            <table>
                <tr>
                    <td>
                        ...
                        <span class="con_etc">
                          ...
                          <span class="hit">
                              	조회&nbsp;<span class="font-color_main"><%= nf_hit %></span>
                          </span>
                        </span>
                    </td>
                </tr>
                ...
                <tr>
                    <td>
                		...
                		%>
                			<input type="button" value="수정하기" onclick="location.href='wfns0103W.jsp?nf_num=<%= nf_num %>&u_id=<%= u_id %>&pageNum=<%= pageNum %>'">
                        	<input type="button" value="삭제하기" onclick="delete_ok()">
                		<%
                		} %>
                        <input type="button" value="목록으로" onclick="location.href='wfns0101R.jsp?u_id=<%= u_id %>&pageNum=<%= pageNum %>'">
                    </td>
                </tr>
            </table>
    	</form>
        ...
        <table id="com_list">
            ...
            <%
            			if(u_id.equals(comm_uid) || re == 1) {
            				%>
            				<input type="button" value="삭제" style="cursor: pointer;" onclick="location.href='wfns0106R.jsp?nf_num=<%= nf_num %>&u_id=<%= u_id %>&pageNum=<%= pageNum %>&comm_num=<%= comm_num %>'">
            				<%
            			}
            %>
                    </div>
                    <!--  대댓글 작업중
                    <input id="com_bt" type="button" value="댓글쓰기 ▼" onclick="com_view()">
                    <div id="com_com" style="display: none;">
                    	<form method="post" action="com_ok.jsp?nf_num=<%= nf_num %>&u_id=<%= u_id %>&pageNum=<%= pageNum %>" name="com_frm">
	                    	<textarea name="comm_content" placeholder="타인을 배려 하는 마음을 담아 댓글을 달아주세요."></textarea>
	                        <input type="button" value="등록" onclick="com_com_wr()">
